...
double g_func(vector<Polygon> cells, vector<Point> points, vector<double> ws, vector<double> lambdas, double f = 1) {
    // function g
    double sum = 0;
    
    for (int i = 1; i < int(points.size()); i ++) {
        double area = Area(cells[i]);
        sum += f * (integral(cells[i], points[i]) - ws[i] * area) + lambdas[i] * ws[i];
    }
    return sum;
}

double g_grad(Polygon cell, double lambda, double f = 1) {
